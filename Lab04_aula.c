//START - Startup.c
//embaixo de To be added by user
//  extern void trata_GPIO_F(void);
//  extern void trata_SysTick(void);

//Colocar na funcao que lida com as interrupts
/* Eh uma funcao com varios IntDefaultHandler,
 * e ao lado de cada linha ha um comentario dizendo com qual evento que ele lida.
 * Substituir nas linhas correspondentes pelo nome da funcao handler desejada.
    IntDefaultHandler do gpioF-> troca por trata_GPIO_F
    e
    IntDefaultHandler do systick -> troca por trata_SysTick(void);
 */
...
#define RCC2_DIV400      0x40000000  // Divide PLL as 400 MHz vs. 200 MHz
...
//Valores possíveis de entrada para função ConfigClock
...
#define portalBASE_GPIO_A   	0x40004000
#define portalBASE_GPIO_B   	0x40005000
#define portalBASE_GPIO_C   	0x40006000
#define portalBASE_GPIO_D  	 	0x40007000
#define portalBASE_GPIO_E  	 	0x40024000
#define portalBASE_GPIO_F   	0x40025000
#define PINO_0      			0x01
#define PINO_1      			0x02
#define PINO_2      			0x04
#define PINO_3      			0x08
...
#define NVIC_EN0                    0xE000E100  //base + offset
#define INT_GPIO_A                  0	//usado na func HabilitaInterrupcao
#define INT_GPIO_B                  1	//usado na func HabilitaInterrupcao
#define INT_GPIO_C                  2	//usado na func HabilitaInterrupcao
#define INT_GPIO_D                  3	//usado na func HabilitaInterrupcao
#define INT_GPIO_E                  4	//usado na func HabilitaInterrupcao
#define INT_GPIO_F                  30 	//usado na func HabilitaInterrupcao  //tabela 2.9 p.104 datasheet

#define nivel_0      			0x02 		//usa na func configIntGPIO
#define nivel_1      			0x03 		//usa na func configIntGPIO
#define borda_descida   		0x00 		//usa na func configIntGPIO
#define borda_sub     			0x01 		//usa na func configIntGPIO
#define ambas_bordas    		0x04 		//usa na func configIntGPIO
...
#define NVIC_ST_CTRL    		0xE000E010 //base + offset
#define NVIC_ST_CTRL_CLK_SRC   	0x04 //no bit 2
#define NVIC_ST_CTRL_ENABLE   	0x01 //no bit 0
#define NVIC_ST_CTRL_INTEN   	0x02 //bit 1


void habilitaGlobal(void){ //item 3 global enable
__asm( " mrs r0, PRIMASK\n"  //coloca conteudo do primask no r0
" cpsie i\n" //so em assembly //hab
" bx lr\n");
}

void HabilitaInterrupcao(uint32_t ui32Interrupcao){
ESC_REG(NVIC_EN0) |= 1<<ui32Interrupcao; //deslocar 1 pra escrever num unico bit
}

void configIntGPIO(uint32_t portal_BASE,uint8_t PINO,uint32_t config){


if((config & 0x01) == 0x01){  //borda
 ESC_REG(portal_BASE + GPIO_O_IEV) |= PINO;
}else{
 ESC_REG(portal_BASE + GPIO_O_IEV) &= ~(PINO);
}

if((config & 0x02) == 0x02){  //borda
 ESC_REG(portal_BASE + GPIO_O_IS) |= PINO;
}else{
 ESC_REG(portal_BASE + GPIO_O_IS) &= ~(PINO);
}

if((config & 0x04) == 0x04){  //borda
...
void Habilita_INT_GPIO(uint32_t portal_BASE,uint8_t PINO){
ESC_REG(portal_BASE + GPIO_O_IM) |= PINO;
}

void Desabilita_INT_GPIO(uint32_t portal_BASE,uint8_t PINO){
ESC_REG(portal_BASE + GPIO_O_IM) &= ~PINO;
}

void Trata_GPIO_F(void){
//testar fonte de interrupcao, qual pino ocasionou a interr
if((ESC_REG(portalBASE_GPIO_F + GPIO_O_RIS) & PINO_4) == PINO_4){ //ver se foi o pin4 q gerou interr
 //executar acoes
}

void limpa_interrup(uint32_t portal_BASE,uint8_t PINO){
ESC_REG(portal_BASE + GPIO_O_ICR) = PINO;
}

void Habilita_SYSTICK(void){
ESC_REG(NVIC_ST_CTRL) |= NVIC_ST_CTRL_ENABLE | NVIC_ST_CTRL_CLK_SRC;
}

void Habilita_INT_SYSTICK(void){
ESC_REG(NVIC_ST_CTRL) |= NVIC_ST_CTRL_INTEN;
}

void Desabilita_INT_SYSTICK(void){
ESC_REG(NVIC_ST_CTRL) &= ~NVIC_ST_CTRL_INTEN;
}

void config_period_SYSTICK(uint32_t period){
ESC_REG(NVIC_ST_RELOAD) = (period - 1);
}

void Trata_SYSTICK(void){
...
int main(void)
{
    ...
    ConfigClock(SYSCTL_SYSDIV_2_5 | SYSCTL_USE_PLL | SYSCTL_XTAL_16MHZ | SYSCTL_OSC_MAIN); //System clock: 80MHz //NAO MEXER
    ...
    configIntGPIO(portalBASE_GPIO_F, PINO_4,borda_descida);
    Habilita_INT_GPIO(portalBASE_GPIO_F, PINO_4);
   
    config_period_SYSTICK(8000000); //10vezes p segundo
    Habilita_INT_SYSTICK();
    Habilita_SYSTICK();  //comeca contagem
